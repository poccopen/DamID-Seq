...
outputfilename = inputGATCfilename.replace(".GATC.bedgraph", ".readnum.bedgraph")
...
# 入力ファイル名の末尾に問題がないときは以下を実行する
	...
	for cl in sorted:
		...
		if (len(pl) != 5 or len(cl) != 5):
			pl = cl
			continue
		# 以下の2条件が同時に成立するときのみ以下の処理を実行する
		# 条件1 plの末尾の要素が"posG_in_GATC"である
		# 条件2 clの末尾の要素が"posG_in_GATC"である
		if (pl[4] == "posG_in_GATC" and cl[4] == "posG_in_GATC"):
			# リード数=0、plの第4カラムの値と入れ替える
			pl[3] = 0
			outputfile.write(str(pl[0]) + "\t" + str(pl[1]) + "\t" + str(pl[2]) + "\t" + str(pl[3]) + "\t" + str(pl[4]) + "_A\n")
			# clの内容をplに代入して次の行に移る
			pl = cl
			continue
		...
		if (pl[4] == "posG_in_GATC" and cl[4] == "TC" and int(cl[1]) == int(pl[1])+2):
			outputfile.write(str(cl[0]) + "\t" + str(cl[1]) + "\t" + str(cl[2]) + "\t" + str(cl[3]) + "\t" + str(cl[4]) + "_B\n")
			# clの内容をplに代入して次の行に移る
			pl = cl
			continue
		# 以下の3条件が同時に成立するときのみ以下の処理を実行する
		# 条件1 plの末尾の要素が"posG_in_GATC"である
		# 条件2 clの末尾の要素が"TC"である
		# 条件3 plの座標とclの座標の差が2でない
		if (pl[4] == "posG_in_GATC" and cl[4] == "TC" and int(cl[1]) != int(pl[1])+2):
			pl[3] = 0
			outputfile.write(str(pl[0]) + "\t" + str(pl[1]) + "\t" + str(pl[2]) + "\t" + str(pl[3]) + "\t" + str(pl[4]) + "_B\n")
			# clの内容をplに代入して次の行に移る
			pl = cl
			continue			
		# 以下の3条件が同時に成立するときのみ以下の処理を実行する
		...
		if (pl[4] == "GATC" and cl[4] == "TC" and int(cl[1]) == int(pl[1])+2):
			# GATCリード数とTCリード数の和を計算し、clの第4カラムの値と入れ替える
			cl[3] = int(pl[3]) + int(cl[3])
			# 最終出力ファイルにclの内容を書き出す
			outputfile.write(str(cl[0]) + "\t" + str(pl[1]) + "\t" + str(pl[2]) + "\t" + str(cl[3]) + "\t" + str(cl[4]) + "_C\n")
			# clの内容をplに代入して次の行に移る
			pl = cl
			continue
		# 以下の3条件が同時に成立するときのみ以下の処理を実行する
		...
		if (pl[4] == "GATC" and cl[4] == "TC" and int(cl[1]) != int(pl[1])+2):
			# 最終出力ファイルにplの内容を書き出す
			outputfile.write(str(pl[0]) + "\t" + str(pl[1]) + "\t" + str(pl[2]) + "\t" + str(pl[3]) + "\t" + str(pl[4]) + "_C\n")
			# clの内容をplに代入して次の行に移る
			pl = cl
			continue
		# 以下の2条件が同時に成立するときのみ以下の処理を実行する
		# 条件1 plの末尾の要素が"GATC"である
		# 条件2 clの末尾の要素が"posG_in_GATC"である
		if (pl[4] == "GATC" and cl[4] == "posG_in_GATC"):
			# 最終出力ファイルにplの内容を書き出す			
			outputfile.write(str(pl[0]) + "\t" + str(pl[1]) + "\t" + str(pl[2]) + "\t" + str(pl[3]) + "\t" + str(pl[4]) + "_D\n")
			# clの内容をplに代入して次の行に移る
			pl = cl
			continue
		# 上記の条件のいずれもが成立しないときはclの内容をplに代入して次の行に移る
		pl = cl
		continue
